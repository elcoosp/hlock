...
fn test_e2e_v7_provenance_roundtrip() {
    ...
    assert!(matches!(
        &res.packages[0].hashes[0].attestation,
        Attestation::ExternalBundleSha256(_)
    ));
    std::fs::remove_file(&temp_path).ok();
}
        ...
        packages: vec![
            Package {
                ...
                resolved_peers: vec![PeerResolution {
                    ...
                    satisfied_by_content_id: fnv::calculate("react@18.0.0"),
    ...
    let lockfile = Lockfile {
        ...
        packages: vec![Package {
            ...
            resolved_peers: vec![PeerResolution {
                ...
                satisfied_by_content_id: fnv::calculate("b@1.0.0"),
...
fn test_e2e_v8_alias_and_cas_roundtrip() {
    ...
    assert_eq!(
        deserialized.sources[0],
        Source::CasHttp("https://cas.example.com/".to_string())
    );
    assert_eq!(
        deserialized.packages[0].logical_name,
        Some("react-v18".to_string())
    );
...
fn test_e2e_ipfs_source_roundtrip() {
    ...
    assert_eq!(
        res.sources[0],
        Source::Ipfs("ipfs://QmXyZ1abcDEF".to_string())
    );
        ...
        packages: vec![
            Package {
                ...
                peer_requirements: vec![
                    ...
                    PeerRequirement {
                        peer_name: "react-native".to_string(),
...
fn test_e2e_platform_extraction_with_real_lockfile() {
    ...
    assert_eq!(
        mac_res.packages.len(),
        4,
        "mac should have 4 packages: {:?}",
        mac_names
    );
    ...
    assert_eq!(
        linux_res.packages.len(),
        4,
        "linux should have 4 packages: {:?}",
        linux_names
    );
    ...
    assert_eq!(
        win_res.packages.len(),
        3,
        "windows should have 3 packages (no native esbuild): {:?}",
        win_names
    );
