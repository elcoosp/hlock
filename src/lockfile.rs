...
use crate::fnv;
use crate::payload::{
    DepPayload, HookHashPayload, PayloadData, PeerReqPayload, PlatformTagPayload, pack_payload,
    unpack_payload,
};
...
fn serialize_diff_text(diff: &LockfileDiff) -> String {
    ...
    let removed: Vec<&Package> = diff
        ...
        .collect();
    let altered: Vec<(&Package, &Package)> = diff
        .changes
        .iter()
        .filter_map(|c| match c {
...
pub fn serialize(lockfile: &mut Lockfile) -> Result<String, Error> {
    ...
    for pkg in lockfile.packages.iter() {
        ...
        let peer_reqs: Vec<PeerReqPayload> = pkg
            .peer_requirements
            .iter()
        ...
        let tags: Vec<PlatformTagPayload> = pkg
            .platform_tags
            .iter()
            .map(|t| {
        ...
        let exports: Vec<crate::payload::ExportPayload> = pkg
            .exports
            .iter()
            .map(|ex| {
                ...
                crate::payload::ExportPayload {
                    identifier: ex.identifier.clone(),
                    hash_algo: algo_id,
                    digest: ex.digest.clone(),
                }
            })
            .collect();

        let artifacts: Vec<crate::payload::ArtifactPayload> = pkg
            .artifacts
            .iter()
            .map(|art| {
        ...
        let hook_hashes: Vec<HookHashPayload> = pkg
            .hook_hashes
            .iter()
            .map(|sh| {
        ...
        let patch_hash: Option<(u8, Vec<u8>)> =
            pkg.patch_hash
                .as_ref()
...
pub fn deserialize(content: &str) -> Result<Lockfile, Error> {
    ...
    for (idx, line) in pkg_content.lines().enumerate() {
        ...
        if line.starts_with("@artifact ") {
            ...
            artifacts.push(ArtifactDirective {
                content_id,
                os_id,
                arch_id,
        ...
        if line.starts_with("@provenance ") {
            ...
            let dep_type = match dep_type_id {
                ...
                _ => {
                    provenance_parse_errors.push(Error::UnknownProvenanceDepType {
                        type_id: dep_type_id,
                    });
                    continue;
                }
            ...
            let source_type = match source_type_id {
                ...
                _ => {
                    provenance_parse_errors.push(Error::UnknownProvenanceSourceType {
            ...
            if let Some(existing) =
                provenance
                    .iter()
                    .position(|p: &crate::provenance::ResolutionProvenance| {
                        p.package_name == prov.package_name
                    })
            {
...
mod tests {
    ...
    fn test_validate_digest_mismatch() {
        ...
        assert!(matches!(
            validate_digest(&with_bad_digest),
    ...
    fn test_validate_digest_duplicate() {
        let content = "@source 0 https://r.com/\n\npkg\tAAAA\n@digest abc\n@digest def\n";
        assert!(matches!(
            validate_digest(content),
            Err(Error::DuplicateDigestDirective)
        ));
    }
    ...
    fn test_provenance_serialize_roundtrip() {
        ...
        assert_eq!(
            deserialized.provenance[0].source_type,
            crate::provenance::ProvenanceSourceType::Registry
        );
    ...
    fn test_provenance_unknown_dep_type() {
        ...
        let without_digest: String = base
            .lines()
            .filter(|l| !l.starts_with("@digest "))
            .collect::<Vec<_>>()
            .join("\n");
